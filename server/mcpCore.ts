...
LLM never chooses the next lesson word. The next word must come from get_next_learning_word, which walks the backend's canonical prepared daily queue after the current word. A completed queue round returns no replacement word; do not invent or randomly select one.
