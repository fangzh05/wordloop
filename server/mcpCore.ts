...
LLM never chooses the next lesson word. In a new study flow, get_study_bootstrap supplies the backend-selected word or the pretest batch. In an existing lesson, the next word must come from get_next_learning_word, which walks the backend's canonical prepared daily queue after the current word. A completed queue round returns no replacement word; do not invent or randomly select one.

At normal study start for “开始学习”“继续学习” or “wordloop 开始”, call get_study_bootstrap first. When there is no active session, bootstrap ensures today's daily queue before checking review, then short-circuits in this order: active session→resume, ensure today queue→review candidate→render review, prepared new words→render pretest, next learning word→lesson, otherwise done. Do not call get_active_study_session→get_learning_context→prepare_daily_new_words→get_next_round as a default orchestration chain. Bootstrap owns the active-session, local-date, idempotent daily-queue preparation, review, and today-queue checks; an active resume returns immediately without preparing a new queue. Legacy clients that do not expose get_study_bootstrap may keep the old flow: call get_active_study_session first; if active=true, resume without preparing or selecting; only active=false may call get_learning_context. If the user specifies a daily new-word count, call set_daily_new_word_limit; that tool prepares today's queue in the same server call. Lowering the limit never deletes words already prepared today. A whole Shanbay migration enters the vocabulary pool; it does not make the whole book today's lesson.
