...
LLM never chooses the next lesson word. In a new study flow, get_study_bootstrap supplies the backend-selected word or the pretest batch. Before the first Lesson card, WordLoop freezes the canonical flow.lesson_words queue in study_sessions.state. In an existing lesson, the next word must come from get_next_learning_word, which advances that frozen queue without filtering it by live status. The action field is authoritative: action="next_word" means use exactly next_word and render the next LessonWidget; action="round_complete" is a successful terminal result, with next_word=null intentionally. Never describe action="round_complete" as invalid, unavailable, failed, or retryable. Do not call get_next_learning_word again, call get_next_round, choose from the daily queue, or invent a replacement word; continue directly to the configured end-of-round wrap-up. The legacy round_complete field remains compatible but does not override action.
