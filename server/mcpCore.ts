...
Formal learning uses the single render_lesson_widget with mode=explain, exercise, or feedback. Teach one backend-specified word at a time. The server validates the word against the active lesson cursor or the canonical next learning word before persisting the card. A new explain payload must include its complete exercise payload, and render_lesson_widget persists it before returning the card, including backend-derived navigation. After pronunciation/listening is complete, render the explain card; after the user starts practice, the Widget calls advance_study_session(lesson_start_exercise) and switches locally without a GPT turn; after grading and record_attempt, render self-contained feedback. The feedback payload must carry the original exercise and server-owned navigation. For an incorrect answer, the first feedback must identify at least one concrete error fragment or location in message and give the reason plus an actionable self-correction direction in explanation; do not return only an error layer or a vague count of errors. Keep reveal_answer=false and omit reference_answer on the first error, so the user can repair the sentence. Only after two consecutive incorrect attempts may the feedback reveal the reference answer and full explanation. A retry calls advance_study_session(lesson_retry) and reuses that exercise without asking GPT for a new one. When navigation.action="next_word", send the exact next_word and render only that word. When navigation.action="round_complete", call advance_study_session({ event: "lesson_complete" }) once, then continue with the configured end-of-round handoff; do not query another word or finish the active session before the handoff is answered and graded. For the required long-sentence wrapup, use the same Widget with mode=exercise or feedback and wrapup=true; use the exact final lesson word only as the server-validated anchor, keep activity_type=sentence, persist the answer with record_attempt, and render feedback before finishing. After that wrap-up is graded, call finish_study_session exactly once and immediately call get_study_bootstrap. lesson_complete is durable and idempotent; resume must preserve the completed UI or the persisted wrap-up card. Keep semantic grading in ChatGPT, but keep all teaching text and user input in the Widget. Example sentences and exercise prompts must be independent new scenes: never translate, reverse-translate, closely paraphrase, or make a minor noun substitution. Derivation practice, listening practice, long-sentence close, 20-word quizzes, and end-of-session free recall all reuse this same Widget.

render_lesson_widget, render_pretest_widget, and render_dictation_widget accept resume=true to restore the active persisted payload. Do not regenerate content already stored in the active session. Call finish_study_session exactly once only after the current round's real long-sentence wrap-up is complete and graded, then immediately call get_study_bootstrap; it clears the active cursor so today's remaining daily items can start a new study session.
