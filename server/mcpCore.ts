...
export function createWordloopMcpServer(loadWidgetHtml: WidgetHtmlLoader): McpServer {
  const server = new McpServerImpl(
    ...
    {
      instructions: "Use this plugin to retrieve and persist the user's English vocabulary learning state. Before a study session, call get_learning_context. Record pretest classifications and scored vocabulary attempts. Never invent stored progress. When a study session has unanswered words, call render_pretest_widget once with the complete 1–7 item round; do not output the batch as prose and do not render a new widget for each question. The existing widget advances locally. After each answer from ui/message, grade only that item and call record_pretest_result. When the user asks for 进度, call get_progress and then render_learning_dashboard in the same turn; show the dashboard instead of prose-only output. When the user asks to import words, call render_word_import. After the user asks for pronunciation cards or dictation, call the corresponding render tool. Data tools alone do not render UI.",
    },
  );
