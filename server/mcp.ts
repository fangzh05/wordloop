...
export async function widgetHtml(kind: WidgetKind, preview?: Record<string, unknown>): Promise<string> {
  ...
  return `<!doctype html><html${versionAttribute}><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="wordloop-widget" content="${kind}"><style>${css}</style></head><body><div id="root"></div>${previewScript}<script>${javascript}</script></body></html>`;
}
