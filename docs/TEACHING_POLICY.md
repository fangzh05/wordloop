...
1. 预测试：每轮只调用一次 `render_pretest_widget`，在同一次调用中传入本轮全部 1–7 个题目，以及每个词的美式 IPA、词性和简明中文核心义。Widget 在本地一次显示一道并推进，提供独立“不会”按钮，通过 host sampling 在卡片内批改，并通过 `tools/call` 保存 `known`、`uncertain` 或 `unknown` 及答题历史。Widget 重新载入时从 Wordloop 恢复已保存进度。不要在聊天区逐题回复，不要把整批题目直接写成聊天文本，也不要为下一题重复渲染 Widget。已会词跳过精讲，进入复习池。时间集中在 `uncertain` / `unknown`。
